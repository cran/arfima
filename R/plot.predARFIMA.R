...
"plot.predarfima" <-
function(x, xlab = NULL, ylab = NULL, main = NULL, ylim = NULL, numback = 5, xlim = NULL, ...) {
	
	op <- par(no.readonly = TRUE)
	z <- x$z
	n <- length(z)
	if(length(xlab)==0) xlab <- "Time"
	if(length(ylab)==0) ylab <- "Mode "
	if(length(main)==0) main <- paste("Time Series and Predictions of ", x$name, sep = "")
	m <- length(x) - 7
	...
	minn <- if(numback > 0) min(ys) else Inf
	maxx <- if(numback > 0) max(ys) else -Inf
	for(i in 1:m) {
		exactPIU <- x[[i]]$Forecast+numSD*x[[i]]$exactSD
		exactPIL <- x[[i]]$Forecast-numSD*x[[i]]$exactSD
		limitPIU <- if(islim) x[[i]]$Forecast+numSD*x[[i]]$limitSD else NULL
		limitPIL <- if(islim) x[[i]]$Forecast-numSD*x[[i]]$limitSD else NULL
	...
	ranger <- maxx - minn
	if(length(ylim)==0) ylim <- c(minn-ranger/20, maxx+ranger/20)
	...
	nf <- layout(matrix(c(1:(m+1)),m+1,1), widths = rep(1, m+1), heights = c(0.5, rep(1, m)), FALSE)
	par(mar = c(0, 0, 0, 0))
...
"plotpredARFIMA" <-
function(xy, xlab = NULL, ylab = NULL, ylim = NULL,...) {
	nn <- length(xy$z)
	plot(x = xy$xs[1:nn], y=xy$z, ylab = ylab, xlab = xlab, xlim = c(xy$xs[1], xy$xs[length(xy$xs)]), ylim = ylim, type = "l", cex.lab = 1.4)
	zlast <- xy$z[nn]
