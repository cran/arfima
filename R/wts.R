...
##Not used right now. 
piwts <- function(phi = 0, theta = 0, phiseas = 0, thetaseas = 0, dfrac = 0, dfs = 0, dint = 0, dseas = 0, period = 0, n = 128, m = 128, maxlag = 128)
{
	multer = 1

	if((length(phi) > 0) && any(phi != 0)) multer <- mult(multer, c(1, -phi))

	if((length(phiseas) > 0) && any(phi != 0) && (period > 0)) multer <- mult(multer, shift(c(1, -phiseas), period))

	if((length(dfrac) > 0) && dfrac != 0) multer <- mult(multer, expand(d = dfrac, n = n))

	if((length(dfs) > 0) && dfs != 0) multer <- mult(multer, expandseas(d = dfs, seas = period, n = n*period))
	...
	if((length(theta) > 0) && any(theta != 0)) multer <- mult(multer, psiwtsAR(theta, m))

	if((length(thetaseas) > 0) && any(thetaseas != 0) && (period > 0)) multer <- mult(multer, psiwtsAR(shift(c(1, thetaseas), period)[-1], m*period))
...
{
	if(dint != round(dint) || dseas != round(dseas) || period != round(period)) stop("non-integers provided where integers needed") 
	if(length(dseas) > 0 && dseas > 0 && period < 2) stop("incompatible period and seasonal d")
