...
predict.arfima <- function(object, n.ahead = 1, newxreg = NULL, predint = 0.95, bootpred = TRUE, B = if(bootpred) 1000 else 0, trex = FALSE, seed = NA,  setmuhat0 = FALSE, cpus = 1, trend  = NULL,...)
{
	npar <- bootpred
	if(!is.null(object$s)&&any(object$s>1)) stop('predict only takes static regression parameters, not full transfer functions/dynamic regressions')
	if(!is.null(object$r)&&any(object$r>1)) stop('predict only takes static regression parameters, not full transfer functions/dynamic regressions')
	myNROW <- function(x) if(is.null(x)) 0 else nrow(x)

	myNCOL <- function(x) if(is.null(x)) 0 else ncol(x)
	
	if(length(seed) == 0) seed <- NA
	
	if(bootpred && is.na(seed)) warning('bootstrap predictions and intervals may not be reproducible without setting the seed(s)')
	...
	if(myNCOL(object$xreg)!=myNCOL(newxreg)) stop('unconformable newxreg and xreg')
	...
	tacvfs <- tacvf(obj = object, xmaxlag = n.ahead, forPred = TRUE, n.ahead = n.ahead+1)
	
	if(nrxreg || (bootpred&&npar)) resids <- resid(object)
	if(nrxreg) {
		newXreg <- newxreg
		res <- resids
	...
	limiting <- FALSE
	for(i in 1:m) {
		zy <- if(!is.null(y)) y else res[[i]]
		nz <- length(zy)
		zz <- c(zy, rep(0, n.ahead))
		muHat <- tacvfs[[i+1]]$muHat
		if(object$differencing) {
			if(setmuhat0) muHat <- 0
			icap <- dint+dseas*period
			zinit <- zy[(nz-icap+1):nz]
			if(dint > 0) zy <- diff(zy, differences = dint)
			if(dseas > 0) zy <- diff(zy, differences = dseas, lag = period)
			if(nrxreg) {
				if(dint > 0) Xreg <- diff(Xreg, differences = dint)
				if(dseas > 0) Xreg <- diff(Xreg, differences = dseas, lag = period)
			}
		}
		else {
			icap <- 0
		...
		znew <- predictWork(y = zy, r = tacvfs[[i+1]]$tacvf, dint = dint, dseas = dseas, period = period, muHat = muHat, n.ahead = n.ahead, trex = trex)
		nz <- length(zy)
		znew$Forecast <- znew$Forecast 
	
		
		if(!is.null(trend)) {
			if(length(trend) < n.ahead) stop("length of trend less than n.ahead")
			if(length(trend) != n.ahead) warning("trend is not the same length as n.ahead")
		...
		if(icap > 0) {
			znew$Forecast <- integ(z = znew$Forecast, zinit = zinit, dint = dint, dseas = dseas, period = period)
		...
		if(bootpred && B > 0) {
			...
			A <- Boot(object$modes[[i]], dint = dint, dseas = dseas, period = period, R = B, n.ahead = n.ahead, n = nz, zinit = zinit, lastpoint = muHat, pred = TRUE, seed = seed)
...
predictWork <-
		function(y, r,dint, dseas, period, muHat, n.ahead, trex = FALSE){

	
	ny <- length(y)

	znew <- if(dseas + dint > 0) TrFore(y,r,muHat,maxLead=n.ahead,getV = FALSE)  else TrFore(y,r,muHat,maxLead=n.ahead)   #correct to use muHat, as it's from the differenced version

	coeffs <- NULL 
	if(dint + dseas > 0) {
		if(trex) coeffs <- rev(Z(l = n.ahead, d = dint, ds = dseas, s = period))
		else coeffs <- wtsforexact(dint = dint, dseas = dseas, period = period, len = n.ahead)
	}
	...
	worker1 <- function(m, value, val) {
		if(m > 0) {
			...
			if(d > 0 && ds > 0) {
				for(i in 1:d) {
					for(j in 1:ds) {
						val <- worker1(m-i-j*s, value*choose(d, i)*choose(ds, j)*(-1)^(i+j+1), val)
					}
				}
			}	
		}
...
print.predARFIMA <- 
function(x, digits = max(6, getOption("digits") - 3), ...) {
	n <- length(x$z)
	if(is.null(x$meanval)) bootpred <- FALSE
	else bootpred <- TRUE
	if(bootpred) {
		lower <- x$lower
		upper <- x$upper
...
TrFore <-
function(z, r, zm, maxLead, getV = TRUE) {
	n <- length(z)

	if (length(r) < (n+maxLead-1))
			stop("error: length(r) must be >= ", n+maxLead-1)
	if(!(maxLead > 0)) stop("maxLead must be > 0")
	...
	if(getV) ans <- list(Forecasts=zf,SDForecasts=sqrt(vf))
	else ans <- list(Forecasts=zf)
	ans
}

exactVals <- function(r, n, maxLead, way = 5) {
	if(length(r) < n + maxLead) stop("r is too short")
	GI <- TrenchInverse(toeplitz(r[1:n]))
