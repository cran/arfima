removeMode <- function(object, num) {
	if(!object$weeded) stop('Please weed the object first before manually removing a mode.')
	m <- length(object$modes)
	if(num>m) stop('incorrect mode specification')
	modes <- object$modes
	modes[[num]] <- NULL
	object$modes <- modes
...
{	
	...
	for(i in 1:length(ans$modes)) {
		...
		coeff <- c(phi, theta, phiseas, thetaseas, dfrac, H, alpha, dfs, Hs, alphas, omega, delta, muHat, logl, sigma2)
		if(length(phi)>1) coeff <- c(coeff, phip)
		if(length(theta)>1) coeff <- c(coeff, thetap)
		if(length(phiseas)>1) coeff <- c(coeff, phiseasp)
		if(length(thetaseas)>1) coeff <- c(coeff, thetaseasp)
		if(x$getHess) {
			ses1 <-  if(is.logical(x$dmean) && x$dmean) NULL else -Inf
			ses <- c(se, ses1, -Inf, -Inf, if(counter > 0) rep(-Inf, counter) else numeric(0))
		...
		star <- if(i %in% ans$index) "*" else ""
		nam <- if(x$getHess) c(paste("Coef.", i, star, ":", sep = ""), paste("SE.", i, star, ":", sep = "")) else paste("Coef.", i, star, ":", sep = "")
		namer <- c(namer, nam)
	}
	...
	if(ans$numcut>0) cat("Wall detection on: ", ans$numcut, " mode(s) were believed to be spurious and were cut\n", sep= "")
	if(any(is.na(coeffs))) cat("NAs come from singular Hessians\n")
	invisible(x)
}
...
residuals.arfima <- resid.arfima <- function(object, reg = FALSE, ...)
{
	...
	xreg <- object$xreg
	if(reg&&is.null(xreg)) stop('Regression residuals requested, but no regression performed.')
	...
	for(i in 1:m) {
		cmode <- ans$modes[[i]]
		if(!reg) ret[[i]] <- cmode$residuals
		else ret[[i]] <- cmode$regResiduals
	}
...
summary.arfima <- function(object, digits = max(4, getOption("digits") - 3), ...)
{
	...
	ans <- object
	cov <- TRUE; cor <- TRUE; FInfo <- TRUE
	cf <- coef(ans)
	vcovs <- vcov(ans, type = "b", summ = TRUE)
	m <- length(ans$modes)
	dmean <- ans$dmean
	cff <- vector("list", m)
	if(FInfo) finfo <- vector("list", m)
	else finfo <- NULL
	if(cov && cor) corrs <- vcov(ans, cor = cor, type = "b", summ = TRUE)
	else if(cov) corrs <- vcovs
	else corrs <- NULL
	logl <- logLik(ans)
	aics <- AIC(logl)
	...
	for(i in 1:m) {
		...
		if((!is.logical(dmean))||(!dmean)) ses <- c(NA, ses)
		if(!vcovs$warnH && !vcovs$warnX) tses <- c(sqrt(diag(vcovs[[i]]$expected)), NA)
		else {
			tses <- NULL
			tseflag <- FALSE
		}	
		if(!is.null(finfo)) finfo[[i]] <- solve(vcovs[[i]]$expected*ans$n)
		zval <- cf[i, ]/ses
		ps <- 2 * pnorm(-abs(zval))
		cff[[i]] <- cbind("Estimate" = cf[i, ], "Std. Error" = ses, 
				"Th. Std. Err." = tses, "z-value" = zval, "Pr(>|z|)" = ps)
		sigmas[i] <- ans$modes[[i]]$sigma2		
	}
	ans1$tseflag <- tseflag
	if(cor) ans1$corrs <- corrs
	else ans1$covs <- corrs
	ans1$sigma2 <- sigmas
	ans1$logl <- logl
	ans1$coef <- cff
	ans1$aics <- aics
...
print.summary.arfima <- function(x, digits = max(6, getOption("digits") - 3), signif.stars = getOption("show.signif.stars"),...) {
	...
	for(i in 1:x$m) {
		...
		cat(paste("sigma^2 estimated as ", format(x$sigma2[i], digits = digits), "; Log-likelihood = ", format(x$logl[i], digits = digits),
		"; AIC = ", format(x$aics[i], digits = digits), "; BIC = ", format(x$bics[i], digits = digits), "\n", sep = ""))
		if(!is.null(x$corrs)) {
...
{
	if(class(object)!="arfima") stop("only arfima objects can be passed to this function")
	if(!object$weeded) {
		warning('The object has not been weeded:  there may be duplicate modes, as well as excessive output. \n For more sensible output, please call weed() on the object')
	}
	ans <- object
	...
	for(i in 1:m) {
		cmode <- ans$modes[[i]]
		if(length(cmode$H)>0||length(cmode$Hs)>0||length(cmode$alpha)>0||length(cmode$alphas)>0) warning('Please note FGN and HD do not have known information matrix components: \n
		reporting matrix without those components')
		ret[[i]] <- iARFIMA(phi = cmode$phi, theta = cmode$theta, phiseas = cmode$phiseas, thetaseas = cmode$thetaseas, 
			dfrac = ifdf, dfs = ifdfs, period = period, exact = !tapprox)
	}
...
logLik.arfima <- function(object, ...)
{
	...
	if(is.numeric(ans$dmean) && !ans$itmean) len <- len - 1 
	
	attr(logl, "df") <- len + 1 #sigma^2
...
coef.arfima <- function(object, tpacf = FALSE, digits = max(4, getOption("digits") - 3), ...)
{
	...
	for(i in 1:m) {
		...
		if(i == 1) {
			...
			if(tpacf) {
				if(length(phip) > 0) nm <- c(nm, paste("phi_p(", 1:length(phi), ")", sep = ""))
			
				if(length(thetap) > 0) nm <- c(nm, paste("theta_p(", 1:length(theta), ")", sep = ""))
			
				if(length(phiseasp) > 0) nm <- c(nm, paste("phi_p.", period, "(", 1:length(phiseas), ")", sep = ""))
			
				if(length(thetaseasp) > 0) nm <- c(nm, paste("theta_p.", period, "(", 1:length(thetaseas), ")", sep = ""))
			}
			else {
				if(length(phi) > 0) nm <- c(nm, paste("phi(", 1:length(phi), ")", sep = ""))
			
				if(length(theta) > 0) nm <- c(nm, paste("theta(", 1:length(theta), ")", sep = ""))
			
				if(length(phiseas) > 0) nm <- c(nm, paste("phi.", period, "(", 1:length(phiseas), ")", sep = ""))
			
				if(length(thetaseas) > 0) nm <- c(nm, paste("theta.", period, "(", 1:length(thetaseas), ")", sep = ""))
			}

			if(length(dfrac) > 0) nm <- c(nm, paste("d.f", sep = ""))
			if(length(H) > 0) nm <- c(nm, paste("H", sep = ""))
			if(length(alpha) > 0) nm <- c(nm, paste("alpha", sep = ""))
			
			if(length(dfs) > 0) nm <- c(nm, paste("d.f.", period, sep = ""))
			if(length(Hs) > 0) nm <- c(nm, paste("H.", period, sep = ""))
			if(length(alphas) > 0) nm <- c(nm, paste("alpha.", period, sep = ""))
			...
			if(itmean) {
				...
				if(is.logical(dmean) && !dmean) nm <- c(nm, "zbar")
				else if(is.logical(dmean) && dmean) nm <- c(nm, paste("Fitted mean", sep = ""))
				else nm <- c(nm, "Set mean")
			}

		}

		if(tpacf) coeff <- c(phip, thetap, phiseasp, thetaseasp, dfrac, H, alpha, dfs, Hs, alphas, omega, delta, muHat)
		else coeff <- c(phi, theta, phiseas, thetaseas, dfrac, H, alpha, dfs, Hs, alphas, omega, delta, muHat)
...
vcov.arfima <- function(object, type = c("b", "o", "e"), cor = FALSE, digits = max(4, getOption("digits") - 3), tapprox = FALSE, summ = FALSE, ...)
{
	if(!object$weeded) {
		warning('The object has not been weeded:  there may be duplicate modes, as well as excessive output. \n For more sensible output, please call weed() on the object')
	}
	...
	if(type == c("b", "o", "e") && !summ) cat('Returning both observed and theoretical covariance matrices: use type = "o" for just
	...
	if((is.logical(ans$dmean) && !ans$dmean)||is.double(ans$dmean)) nm <- nm[-length(nm)]
	period <- ans$period
	warnH <- warnX <- FALSE
	if(type %in% c("b", "o")) {
		for(i in 1:m) {
			...
			if(cor) {
				...
				for(j in 1:nrow(temp)) {
					for(k in 1:ncol(temp)) {
						temp[j, k] <- ret[[i]]$observed[j, k]/sqrt(ret[[i]]$observed[j, j]*ret[[i]]$observed[k, k])
					}
				}
				ret[[i]]$observed <- temp
			}
	...
	if(type %in% c("b", "e")) {
		if((length(ans$modes[[1]]$H) > 0 || length(ans$modes[[1]]$Hs)>0|| length(ans$modes[[1]]$alpha) > 0 || length(ans$modes[[1]]$alphas)>0)) {
			if(!summ) warning('The expected Fisher Information matrix has no known closed form for FGN or HD processes.\n
				Reporting this matrix for the non-FGN/non-HD processes: however, results may be skewed')
			warnH <- TRUE
		}		
		if(!is.null(xreg)) {
			if(!summ) warning('The expected Fisher Information matrix does not include regression parameters.\n
		...
		for(i in 1:m) {
			...
			ret[[i]]$expected <- solve(iARFIMA(phi = cmode$phi, theta = cmode$theta, phiseas = cmode$phiseas, thetaseas = cmode$thetaseas, 
				dfrac = ifdf, dfs = ifdfs, period = period, exact = !tapprox))/ans$n
			if(cor) {
				temp <- ret[[i]]$expected
				for(j in 1:nrow(temp)) {
					for(k in 1:ncol(temp)) {
						temp[j, k] <- ret[[i]]$expected[j, k]/sqrt(ret[[i]]$expected[j, j]*ret[[i]]$expected[k, k])
					}
				}
				ret[[i]]$expected <- temp
			}
...
getnames <- function(ans) {
	...
	if(class(ans)!="arfima") stop("the getnames function only works for arfima objects")
